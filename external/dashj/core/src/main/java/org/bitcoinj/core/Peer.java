...
public class Peer extends PeerSocketHandler {
    ...
    private final CopyOnWriteArrayList<ListenerRegistration<HeadersDownloadedEventListener>> headersDownloadedEventListeners = new CopyOnWriteArrayList<>();
    ...
    private final CopyOnWriteArrayList<ListenerRegistration<HeadersDownloadStartedEventListener>> headersDownloadStartedEventListeners = new CopyOnWriteArrayList<>();
    ...
    private final CopyOnWriteArrayList<ListenerRegistration<MasternodeListDownloadedListener>> masternodeListDownloadedListeners = new CopyOnWriteArrayList<>();
    // Whether to try and download blocks and transactions from this peer. Set to
    // false by PeerGroup if not the
    // primary peer. This is to avoid redundant work and concurrency problems with
    // downloading the same chain
    // in parallel.
    ...
    // useful for setting our "user agent"
    ...
    // so we can use this to calculate the height of the peers chain, by adding it
    // to the initial height in the version
    ...
    // Whether we are currently downloading headers only or block bodies. Starts at
    // true. If the fast catchup time is
    // set AND our best block is before that date, switch to false until block
    // headers beyond that point have been
    // received at which point it gets set to true again. This isn't relevant unless
    // vDownloadData is true.
    @GuardedBy("lock")
    ...
    // The current Bloom filter set on the connection, used to tell the remote peer
    // what transactions to send us.
    ...
    // If non-null, we should discard incoming filtered blocks because we ran out of
    // keys and are awaiting a new filter
    // to be calculated by the PeerGroup. The discarded block hashes should be added
    // here so we can re-request them
    // once we've recalculated and resent a new filter.
    @GuardedBy("lock")
    @Nullable
    private List<Sha256Hash> awaitingFreshFilter;
    // Keeps track of things we requested internally with getdata but didn't receive
    // yet, so we can avoid re-requests.
    // It's not quite the same as getDataFutures, as this is used only for getdatas
    // done as part of downloading
    // the chain and so is lighter weight (we just keep a bunch of hashes not
    // futures).
    //
    // It is important to avoid a nasty edge case where we can end up with parallel
    // chain downloads proceeding
    // simultaneously if we were to receive a newly solved block whilst parts of the
    // chain are streaming to us.
    private final HashSet<Sha256Hash> pendingBlockDownloads = new HashSet<>();
    // Keep references to TransactionConfidence objects for transactions that were
    // announced by a remote peer, but
    ...
    // the confidence object held inside it, and it's then up to the event listeners
    ...
    // When an API user explicitly requests a block or transaction from a peer, the
    // InventoryItem is put here
     ...
     *
     * <p>
     * Note that this does <b>NOT</b> make a connection to the given remoteAddress,
     * it only creates a handler for a
     * connection. If you want to create a one-off connection, create a Peer and
     * pass it to
     * {@link NioClientManager#openConnection(SocketAddress, StreamConnection)}
     * or
     * {@link NioClient#NioClient(SocketAddress, StreamConnection, int)}.
     * </p>
     *
     * <p>
     * The remoteAddress provided should match the remote address of the peer which
     * is being connected to, and is
     * used to keep track of which peers relayed transactions and offer more
     * descriptive logging.
     * </p>
     */
    ...
    /**
     * <p>
     * Construct a peer that reads/writes from the given block chain. Transactions
     * stored in a {@link TxConfidenceTable}
     * will have their confidence levels updated when a peer announces it, to
     * reflect the greater likelihood that
     ...
     */
    public Peer(NetworkParameters params, VersionMessage ver, PeerAddress remoteAddress,
            @Nullable AbstractBlockChain chain, @Nullable AbstractBlockChain headers, long requiredServices,
     ...
     * creates a VersionMessage for you from
     * the given software name/version strings, which should be something like
     * "MySimpleTool", "1.0" and which will tell
     * the remote node to relay transaction inv messages before it has received a
     * filter.
     * </p>
     *
     * <p>
    ...
    /**
     * Registers a listener that is called when a transaction is broadcast across
     * the network
     */
    ...
    protected void timeoutOccurred() {
        ...
        if (!connectionOpenFuture.isDone()) {
            connectionClosed(); // Invoke the event handlers to tell listeners e.g. PeerGroup that we never
                                // managed to connect.
        }
    }
    ...
    public void connectionOpened() {
        // Announce ourselves. This has to come first to connect to clients beyond
        // v0.3.20.2 which wait to hear
        // from us until they send their version message back.
        PeerAddress address = getAddress();
        log.info("Announcing to {} with protocol {} as: {}", address == null ? "Peer" : address.toSocketAddress(),
                versionMessage.clientVersion, versionMessage.subVer);
        sendMessage(versionMessage);
        connectionOpenFuture.set(this);
        // When connecting, the remote peer sends us a version message with various bits
        // of
        // useful data in it. We need to know the peer protocol version before we can
        // talk to it.
    }

    /**
     * Provides a ListenableFuture that can be used to wait for the socket to
     * connect. A socket connection does not
    ...
    protected void processMessage(Message m) throws Exception {
         ...
         * if(count % 100 == 0)
         * {
         * log.info("[bandwidth] " + (dataReceived / 1024 / 1024) + " MiB in "
         * +(current-startTime)/1000 + " s:" + (dataReceived /
         * 1024)/(current-startTime)*1000 + " KB/s");
         * }
         * count++;
         * }
         */

        // Allow event listeners to filter the message stream. Listeners are allowed to
        // drop messages by
        // returning null.
        ...
        // If we are in the middle of receiving transactions as part of a filtered block
        // push from the remote node,
        ...
        if (m instanceof Ping) {
            ...
            // This is sent to us when we did a getdata on some transactions that aren't in
            // the peers memory pool.
            // Because NotFoundMessage is a subclass of InventoryMessage, the test for it
            ...
            // we should save them in the wallet so we don't put too much load on the seed
            // nodes and can
    ...
    protected void processInstantSendLock(InstantSendLock islock) {
        if (currentFilteredBlock != null) {
            if (!currentFilteredBlock.getTransactionHashes().contains(islock.getTxId())) {
                // Got an islock that didn't fit into the filtered block, so we must have
                // received everything.
                endFilteredBlock(currentFilteredBlock);
                currentFilteredBlock = null;
            } else {
                // if this transaction is more than 24 hours old, don't process the islock
                if (currentFilteredBlock.getBlockHeader().getTimeSeconds() + TimeUnit.DAYS.toSeconds(1) > System
                        .currentTimeMillis()) {
                    return;
                }
            }
    ...
    private void processVersionMessage(VersionMessage peerVersionMessage) throws ProtocolException {
        ...
        log.info(toString());
        // bitcoinj is a client mode implementation. That means there's not much point
        // in us talking to other client
        // mode nodes because we can't download the data from them we need to
        // find/verify transactions. Some bogus
        // implementations claim to have a block chain in their services field but then
        // report a height of zero, filter
        // them out here.
    ...
    protected void startFilteredBlock(FilteredBlock m) {
        // Filtered blocks come before the data that they refer to, so stash it here and
        // then fill it out as
        // messages stream in. We'll call endFilteredBlock when a non-tx message arrives
        // (eg, another
        // FilteredBlock) or when a tx that isn't needed by that block is found. A ping
    ...
    protected void processNotFoundMessage(NotFoundMessage m) {
        // This is received when we previously did a getdata but the peer couldn't find
        // what we requested in it's
        // memory pool. Typically, because we are downloading dependencies of a relevant
        // transaction and reached
        // the bottom of the dependency tree (where the unconfirmed transactions connect
        // to transactions that are
        // in the chain).
        //
        // We go through and cancel the pending getdata futures for the items we were
        // told weren't found.
    ...
    protected void processAlert(AlertMessage m) {
        try {
            ...
            // Signature checking can FAIL on Android platforms before Gingerbread
            // apparently due to bugs in their
            // BigInteger implementations! See
            // https://github.com/bitcoinj/bitcoinj/issues/526 for discussion. As
    ...
    protected void processHeaders(HeadersMessage m) throws ProtocolException {
        ...
        //
        // This method can run if a peer just randomly sends us a "headers" message
        // (should never happen), or more
        ...
        if (vDownloadHeaders && headerChain != null) {
            try {
                ...
                StoredBlock lastHeader = headerChain.getChainHead();// new StoredBlock(previous, work,
        ...
        // this is to avoid cases were the headers were requested when they are not
        // needed
        ...
        try {
            ...
            for (int i = 0; i < m.getBlockHeaders().size(); i++) {
                ...
                if (!passedTime && !reachedTop) {
                    ...
                    if (blockChain.add(header)) {
                        ...
                        // genesis block yet.
                        // That must mean that the peer is buggy or malicious because we specifically
                        // requested for
            ...
            // We added all headers in the message to the chain. Request some more if we got
            // up to the limit, otherwise
    ...
    public void startMasternodeListDownload() {
        try {
            StoredBlock masternodeListBlock = headerChain.getChainHead().getHeight() != 0
                    ? headerChain.getBlockStore()
                            .get(headerChain.getBestChainHeight() - SigningManager.SIGN_HEIGHT_OFFSET)
                    : blockChain.getBlockStore()
                            .get(blockChain.getBestChainHeight() - SigningManager.SIGN_HEIGHT_OFFSET);
    ...
    protected void processTransaction(final Transaction tx) throws VerificationException {
        ...
        try {
            ...
            // Label the transaction as coming in from the P2P network (as opposed to being
            // created by us, direct import,
            ...
            // data into the heap, meaning
            // we can stop holding a reference to the confidence object ourselves. It's up
            // to event listeners on the
            // Peer to stash the tx object somewhere if they want to keep receiving updates
            // about network propagation
            // and so on.
            ...
            if (currentFilteredBlock != null) {
                if (!currentFilteredBlock.provideTransaction(tx)) {
                    // Got a tx that didn't fit into the filtered block, so we must have received
                    // everything.
                    endFilteredBlock(currentFilteredBlock);
                    currentFilteredBlock = null;
                }
                // Don't tell wallets or listeners about this tx as they'll learn about it when
                // the filtered block is
                // fully downloaded instead.
                return;
            }

            // It's a broadcast transaction. Tell all wallets about this tx so they can
            // check if it's relevant or not.
            for (final Wallet wallet : wallets) {
                try {
                    if (wallet.isPendingTransactionRelevant(tx)) {
                        if (vDownloadTxDependencyDepth > 0) {
                            // This transaction seems interesting to us, so let's download its dependencies.
                            // This has
                            // several purposes: we can check that the sender isn't attacking us by engaging
                            // in protocol
                            // abuse games, like depending on a time-locked transaction that will never
                            // confirm, or
                            // building huge chains of unconfirmed transactions (again - so they don't
                            // confirm and the
                            // money can be taken back with a Finney attack). Knowing the dependencies also
                            // lets us
                            // store them in a serialized wallet so we always have enough data to
                            // re-announce to the
                            // network and get the payment into the chain, in case the sender goes away and
                            // the network
                            // starts to forget.
                            //
                            // TODO: Not all the above things are implemented.
                            //
                            // Note that downloading of dependencies can end up walking around 15 minutes
                            // back even
                            // through transactions that have confirmed, as getdata on the remote peer also
                            // checks
                            // relay memory not only the mempool. Unfortunately we have no way to know that
                            // here. In
        ...
        // Tell all listeners about this tx so they can decide whether to keep it or
        // not. If no listener keeps a
        // reference around then the memory pool will forget about it after a while too
     ...
     * transaction depends on, recursively.
     * Only transactions in peers memory pools are included; the recursion stops at
     * transactions that are in the
     * current best chain. So it doesn't make much sense to provide a tx that was
     * already in the best chain and
     * a precondition checks this.
     * </p>
     *
     * <p>
     * For example, if tx has 2 inputs that connect to transactions A and B, and
     * transaction B is unconfirmed and
     * has one input connecting to transaction C that is unconfirmed, and
     * transaction C connects to transaction D
     ...
     * unconfirmed transaction might take
     * to confirm, by checking for unexpectedly time locked transactions, unusually
     * deep dependency trees or fee-paying
     * transactions that depend on unconfirmed free transactions.
     * </p>
     *
     * <p>
     * Note that dependencies downloaded this way will not trigger the onTransaction
     * method of event listeners.
     * </p>
     */
    public ListenableFuture<List<Transaction>> downloadDependencies(Transaction tx) {
        ...
        // future will be invoked when the entire dependency tree has been walked and
    ...
    // The marker object in the future returned is the same as the parameter. It is
    ...
    protected ListenableFuture<Object> downloadDependenciesInternal(final int maxDepth, final int depth,
        ...
        // We want to recursively grab its dependencies. This is so listeners can learn
        // important information like
        // whether a transaction is dependent on a timelocked transaction or has an
        // unexpectedly deep dependency tree
        // or depends on a no-fee transaction.

        // We may end up requesting transactions that we've already downloaded and
        // thrown away here.
        ...
        try {
            ...
            Futures.addCallback(successful, new FutureCallback<List<Transaction>>() {
                ...
                public void onSuccess(List<Transaction> transactions) {
                    // Once all transactions either were received, or we know there are no more to
                    // come ...
                    // Note that transactions will contain "null" for any positions that weren't
                    // successful.
    ...
    protected void processBlock(Block m) {
        ...
        pendingBlockDownloads.remove(m.getHash());
        try {
            // Otherwise it's a block sent to us because the peer thought we needed it, so
            ...
            if (blockChain.add(m)) {
                ...
                // filtering can work.
                //
                // We only do this if we are not currently downloading headers. If we are then
    ...
    protected void endFilteredBlock(FilteredBlock m) {
        ...
        // Note that we currently do nothing about peers which maliciously do not
        // include transactions which
        ...
        try {
            ...
            // The FilteredBlock m here contains a list of hashes, and may contain
            // Transaction objects for a subset
            // of the hashes (those that were sent to us by the remote peer). Any hashes
            // that haven't had a tx
            // provided in processTransaction are ones that were announced to us previously
            // via an 'inv' so the
            // assumption is we have already downloaded them and either put them in the
            // wallet, or threw them away
            // for being false positives.
            //
            // TODO: Fix the following protocol race.
            // It is possible for this code to go wrong such that we miss a confirmation. If
            // the remote peer announces
            // a relevant transaction via an 'inv' and then it immediately announces the
            // block that confirms
            // the tx before we had a chance to download it+its dependencies and provide
            // them to the wallet, then we
            // will add the block to the chain here without the tx being in the wallet and
            // thus it will miss its
            // confirmation and become stuck forever. The fix is to notice that there's a
            // pending getdata for a tx
            // that appeared in this block and delay processing until it arrived ... it's
            // complicated by the fact that
            // the data may be requested by a different peer to this one.

            // Ask each wallet attached to the peer/blockchain if this block exhausts the
            // list of data items
            // (keys/addresses) that were used to calculate the previous filter. If so, then
            // it's possible this block
            // is only partial. Check for discarding first so we don't check for exhaustion
            // on blocks we already know
            // we're going to discard, otherwise redundant filters might end up being queued
            // and calculated.
            lock.lock();
            try {
                if (awaitingFreshFilter != null) {
                    log.info("Discarding block {} because we're still waiting for a fresh filter", m.getHash());
                    // We must record the hashes of blocks we discard because you cannot do
                    // getblocks twice on the same
                    // range of blocks and get an inv both times, due to the codepath in Dash Core
                    // hitting
                    // CPeer::PushInventory() which checks CPeer::setInventoryKnown and thus
                    // deduplicates.
                    ...
                    // Yes, so we must abandon the attempt to process this block and any further
                    // blocks we receive,
                    // then wait for the Bloom filter to be recalculated, sent to this peer and for
                    // the peer to acknowledge
                    // that the new filter is now in use (which we have to simulate with a
                    // ping/pong), and then we can
                    // safely restart the chain download with the new filter that contains a new set
                    // of lookahead keys.
            ...
            if (AbstractBlockChain.ALLOW_MISSING_PARENTS || AbstractBlockChain.FAST_API_10POW_ENABLED) {
                ...
                try {
                    ...
                    log.warn("Failed to add header after merkle failure", ex);
                }
            } else {
                // We don't want verification failures to kill the thread.
            ...
            // We pruned away some of the data we need to properly handle this block. We
            // need to request the needed
    ...
    private void invokeOnBlocksDownloaded(final Block block, @Nullable final FilteredBlock fb) {
        // It is possible for the peer block height difference to be negative when
        // blocks have been solved and broadcast
        // since the time we first connected to the peer. However, it's weird and
        // unexpected to receive a callback
        // with negative "blocks left" in this case, so we clamp to zero so the API user
    ...
    protected void processInv(InventoryMessage inv) {
        ...
        if (transactions.size() == 0 && blocks.size() == 1) {
            // Single block announcement. If we're downloading the chain this is just a
            // tickle to make us continue
            // (the block chain download protocol is very implicit and not well thought
            // out). If we're not downloading
            // the chain then this probably means a new block was solved and the peer
            // believes it connects to the best
        ...
        while (it.hasNext()) {
            ...
            // Only download the transaction if we are the first peer that saw it be
            // advertised. Other peers will also
            // see it be advertised in inv packets asynchronously, they co-ordinate via the
            // memory pool. We could
            // potentially download transactions faster by always asking every peer for a tx
            // when advertised, as remote
            // peers run at different speeds. However to conserve bandwidth on mobile
            // devices we try to only download a
            // transaction once. This means we can miss broadcasts if the peer disconnects
            // between sending us an inv and
            // sending us the transaction: currently we'll never try to re-fetch after a
            // timeout.
            //
            ...
            if (conf.numBroadcastPeers() > 1) {
                ...
                // Register with the garbage collector that we care about the confidence data
                // for a while.
        ...
        if (context.instantSendManager != null && context.instantSendManager.isNewInstantSendEnabled() &&
                context.masternodeSync != null
                && context.masternodeSync.hasSyncFlag(MasternodeSync.SYNC_FLAGS.SYNC_INSTANTSENDLOCKS)) {
        ...
        // The ChainLock (CLSIG)
        if (/*
             * context.sporkManager != null &&
             * context.sporkManager.isSporkActive(SporkManager.SPORK_19_CHAINLOCKS_ENABLED)
             * &&
             */
        context.masternodeSync != null
                && context.masternodeSync.hasSyncFlag(MasternodeSync.SYNC_FLAGS.SYNC_CHAINLOCKS)) {
        ...
        if (context.masternodeSync != null
                && context.masternodeSync.hasSyncFlag(MasternodeSync.SYNC_FLAGS.SYNC_SPORKS)) {
            it = sporks.iterator();
            while (it.hasNext()) {
                InventoryItem item = it.next();
                getdata.addItem(item);
        ...
        if (context.masternodeSync != null
                && context.masternodeSync.syncFlags.contains(MasternodeSync.SYNC_FLAGS.SYNC_GOVERNANCE)) {
        ...
        // If we are requesting filteredblocks we have to send a ping after the getdata
        // so that we have a clear
        // end to the final FilteredBlock's transactions (in the form of a pong) sent to
        // us
        ...
        try {
            if (blocks.size() > 0 && downloadData && blockChain != null) {
                // Ideally, we'd only ask for the data here if we actually needed it. However
                // that can imply a lot of
                // disk IO to figure out what we've got. Normally peers will not send us inv for
                // things we already have
                // so we just re-request it here, and if we get duplicates the block chain /
                // wallet will filter them out.
                for (InventoryItem item : blocks) {
                    if (blockChain.isOrphan(item.hash) && downloadBlockBodies) {
                        // If an orphan was re-advertised, ask for more blocks unless we are not
                        ...
                        // Don't re-request blocks we already requested. Normally this should not
                        // happen. However there is
                        // an edge case: if a block is solved and we complete the
                        // inv<->getdata<->block<->getblocks cycle
                        // whilst other parts of the chain are streaming in, then the new getblocks
                        // request won't match the
                        // previous one: whilst the stopHash is the same (because we use the orphan
                        // root), the start hash
                        // will be different and so the getblocks req won't be dropped as a duplicate.
                        // We'll end up
                        // requesting a subset of what we already requested, which can lead to parallel
                        // chain downloads
                        // and other nastiness. So we just do a quick removal of redundant getdatas here
                        // too.
                        //
                        // Note that as of June 2012 Dash Core won't actually ever interleave blocks
                        // pushed as
                        // part of chain download with newly announced blocks, so it should always be
                        // taken care of by
                        // the duplicate check in blockChainDownloadLocked(). But Dash Core may change
                        // in future so
                ...
                // If we're downloading the chain, doing a getdata on the last block we were
                // told about will cause the
                // peer to advertize the head block to us in a single-item inv. When we download
                // THAT, it will be an
                // orphan block, meaning we'll re-enter blockChainDownloadLocked() to trigger
                // another getblocks between the
                // current best block we have and the orphan block. If more blocks arrive in the
                // meantime they'll also
                // become orphan.
            }
        } finally {
            lock.unlock();
    ...
    /**
     * Asks the connected peer for the block of the given hash, and returns a future
     * representing the answer.
     * If you want the block right away and don't mind waiting for it, just call
     * .get() on the result. Your thread
     ...
     * Transactions in the chain cannot be
     * retrieved this way because peers don't have a transaction ID to
     * transaction-pos-on-disk index, and besides,
     * in future many peers will delete old transaction data they don't need.
     */
    @SuppressWarnings("unchecked")
    // The 'unchecked conversion' warning being suppressed here comes from the
    // sendSingleGetData() formally returning
    // ListenableFuture instead of ListenableFuture<Transaction>. This is okay as
    // sendSingleGetData() actually returns
    // ListenableFuture<Transaction> in this context. Note that sendSingleGetData()
    // is also used for Blocks.
    ...
    /**
     * Sends a getaddr request to the peer and returns a future that completes with
     * the answer once the peer has replied.
     */
     ...
     * created before the given date. Any
     * transactions relevant to the wallet will therefore not be found, but if you
     * know your wallet has no such
     * transactions it doesn't matter and can save a lot of bandwidth and processing
     * time. Note that the times of blocks
     * isn't known until their headers are available and they are requested in
     * chunks, so some headers may be downloaded
     * twice using this scheme, but this optimization can still be a large win for
     * newly created wallets.
     *
     * @param secondsSinceEpoch Time in seconds since the epoch or 0 to reset to
     *                          always downloading block bodies.
     */
    ...
    /**
     * Links the given wallet to this peer. If you have multiple peers, you should
     * use a {@link PeerGroup} to manage
     * them and use the {@link PeerGroup#addWallet(Wallet)} method instead of
     * registering the wallet with each peer
    ...
    private void blockChainHeaderDownloadLocked(Sha256Hash toHash) {
        ...
        if ((checkNotNull(headerChain).getChainHead().getHeight() == 0
                && checkNotNull(blockChain).getChainHead().getHeight() != 0) ||
                headerChain.getChainHead().getHeight() < blockChain.getChainHead().getHeight()) {
     ...
     * deemed to be complete once we've
     * downloaded the same number of blocks that the peer advertised having in its
     * version handshake message.
     */
    public void startBlockChainDownload() {
        setDownloadData(true);
        // TODO: peer might still have blocks that we don't have, and even have a
        // heavier
        ...
        if (blocksLeft >= 0) {
            ...
            // When we just want as many blocks as possible, we can set the target hash to
            // zero.
            lock.lock();
            try {
                blockChainDownloadLocked(Sha256Hash.ZERO_HASH);
    ...
    /**
     * Sends the peer a ping message and returns a future that will be invoked when
     * the pong is received back.
     * The future provides a number which is the number of milliseconds elapsed
     * between the ping and the pong.
     * Once the pong is received the value returned by
     * {@link Peer#getLastPingTime()} is
     * updated.
     * 
     * @throws ProtocolException if the peer version is too low to support
     *                           measurable pings.
     */
    ...
    /**
     * Returns the elapsed time of the last ping/pong cycle. If {@link Peer#ping()}
     * has never
     * been called or we did not hear back the "pong" message yet, returns
     * {@link Long#MAX_VALUE}.
     */
     ...
     * {@link Peer#ping()} has never
     * been called or we did not hear back the "pong" message yet, returns
     * {@link Long#MAX_VALUE}. The moving average
     * window is 5 buckets.
     */
    ...
    /**
     * Returns the difference between our best chain height and the peers, which can
     * either be positive if we are
    ...
    public int getPeerBlockHeightDifference() {
        ...
        // chainHeight should not be zero/negative because we shouldn't have given the
        // user a Peer that is to another
        // client-mode node, nor should it be unconnected. If that happens it means the
        // user overrode us somewhere or
    ...
    /**
     * Returns true if this peer will try and download things it is sent in "inv"
     * messages. Normally you only need
     ...
     * hears about. Normally, only one
     * peer should download missing blocks. Defaults to true. Changing this value
     * from false to true may trigger
     * a request to the remote peer for the contents of its memory pool, if Bloom
     * filtering is active.
     */
     ...
     * protocol version lower than this, it
     ...
     * {@link BloomFilter} object to be sent to the
     * remote peer and if either a memory pool has been set using the constructor or
     * the
     * vDownloadData property is true, a {@link MemoryPoolMessage} is sent as well
     * to trigger downloading of any
     * pending transactions that may be relevant.
     * </p>
     *
     * <p>
     * The Peer does not automatically request filters from any wallets added using
     * {@link Peer#addWallet(Wallet)}.
     * This is to allow callers to avoid redundantly recalculating the same filter
     * repeatedly when using multiple peers
     ...
     * ignored. Once set you presently cannot
     ...
     * Sets a Bloom filter on this connection. This will cause the given
     * {@link BloomFilter} object to be sent to the
     * remote peer and if requested, a {@link MemoryPoolMessage} is sent as well to
     * trigger downloading of any
     * pending transactions that may be relevant.
     * </p>
     *
     * <p>
    ...
    private void maybeRestartChainDownload() {
        ...
        try {
            if (awaitingFreshFilter == null)
                return;
            if (!vDownloadData) {
                // This branch should be harmless but I want to know how often it happens in
                // reality.
                log.warn("Lost download peer status whilst awaiting fresh filter.");
                return;
            }
            // Ping/pong to wait for blocks that are still being streamed to us to finish
            ...
            ping().addListener(new Runnable() {
                ...
                public void run() {
                    ...
                    // It's to ensure we know when the end of a filtered block stream of txns is,
                    // but we should just be
     ...
     * {@link Peer#setBloomFilter(BloomFilter)}. Bloom filters tell
    ...
    /**
     * Sends a query to the remote peer asking for the unspent transaction outputs
     * (UTXOs) for the given outpoints,
     * with the memory pool included. The result should be treated only as a hint:
     * it's possible for the returned
     * outputs to be fictional and not exist in any transaction, and it's possible
     * for them to be spent the moment
     * after the query returns. <b>Most peers do not support this request. You will
     * need to connect to Bitcoin XT
     ...
     * Sends a query to the remote peer asking for the unspent transaction outputs
     * (UTXOs) for the given outpoints.
     * The result should be treated only as a hint: it's possible for the returned
     * outputs to be fictional and not
     * exist in any transaction, and it's possible for them to be spent the moment
     * after the query returns.
     * <b>Most peers do not support this request. You will need to connect to
     * Bitcoin XT peers if you want
     * this to work.</b>
     *
     * @param includeMempool If true (the default) the results take into account the
     ...
     * backwards through transaction dependencies
     * before handing the transaction off to the wallet. The wallet can do risk
     * analysis on pending/recent transactions
     ...
     * through transaction dependencies
     * before handing the transaction off to the wallet. The wallet can do risk
     * analysis on pending/recent transactions
     * to try and discover if a pending tx might be at risk of double spending.
     */
    ...
    public int getMasternodeListCount() {
        return masternodeListCount;
    }

    public void setMasternodeListCount(int count) {
        masternodeListCount = count;
    }
