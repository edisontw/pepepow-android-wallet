 ...
 * together, and knows how to verify that
 ...
 *
 * <p>
 * An AbstractBlockChain implementation must be connected to a
 * {@link BlockStore} implementation. The chain object
 * by itself doesn't store any data, that's delegated to the store. Which store
 * you use is a decision best made by
 * reading the getting started guide, but briefly, fully validating block chains
 * need fully validating stores. In
 ...
 * BlockChain that does/doesn't do full
 * verification. It verifies headers and is implements most of what is required
 * to implement SPV mode, but
 ...
 * {@link BlockChain}, which is the simplest
 * class and implements <i>simplified payment verification</i>. This is a
 * lightweight and efficient mode that does
 * not verify the contents of blocks, just their headers. A
 * {@link FullPrunedBlockChain} paired with a
 * {@link H2FullPrunedBlockStore} implements full verification, which is
 * equivalent to
 * Dash Core. To learn more about the alternative security models, please
 * consult the articles on the
 * website.
 * </p>
 *
 * <b>Theory</b>
 ...
 * mostly as a list of {@link Block}s.
 * When multiple new head blocks are found simultaneously, there are multiple
 * stories of the economy competing to become
 * the one true consensus. This can happen naturally when two miners solve a
 * block within a few seconds of each other,
 * or it can happen when the chain is under attack.
 * </p>
 *
 * <p>
 * A reference to the head block of the best known chain is stored. If you can
 * reach the genesis block by repeatedly
 * walking through the prevBlock pointers, then we say this is a full chain. If
 * you cannot reach the genesis block
 * we say it is an orphan chain. Orphan chains can occur when blocks are solved
 * and received during the initial block
 * chain download, or if we connect to a peer that doesn't send us blocks in
 * order.
 * </p>
 *
 * <p>
 * A reorganize occurs when the blocks that make up the best known chain change.
 * Note that simply adding a
 * new block to the top of the best chain isn't a reorganize, but that a
 * reorganize is always triggered by adding
 * a new block that connects to some other (non best head) block. By "best" we
 * mean the chain representing the largest
 * amount of work done.
 * </p>
 *
 * <p>
 * Every so often the block chain passes a difficulty transition point. At that
 * time, all the blocks in the last
...
public abstract class AbstractBlockChain {
    ...
    private static final int SKIP_HEADER_VERIFICATION_AFTER_HEIGHT = Integer
            .getInteger("bitcoinj.skipHeaderVerificationAfterHeight", -1);

    /**
     * Allow adding blocks with missing parents. This is used for FAST_API_10POW
     * sync.
     * When true, if a block's parent is not found, it will be linked to the current
     * chain head.
     */
    public static boolean ALLOW_MISSING_PARENTS = false;

    /**
     * Relax difficulty verification for FAST_API_10POW sync.
     * When true, difficulty mismatches are logged as warnings instead of throwing
     * exceptions.
     */
     ...
     * <p>
     *
     * Following this one down to the genesis block produces the story of the
     * economy from the creation of Bitcoin
     * until the present day. The chain head can change if a new set of blocks is
     * received that results in a chain of
     * greater work than the one obtained by following this one down. In that case a
     * reorganize is triggered,
    ...
    // TODO: Scrap this and use a proper read/write for all of the block chain
    // objects.
    // The chainHead field is read/written synchronized with this object rather than
    // BlockChain. However writing is
    // also guaranteed to happen whilst BlockChain is synchronized (see
    // setChainHead). The goal of this is to let
    // clients quickly access the chain head even whilst the block chain is
    // downloading and thus the BlockChain is
    ...
    /**
     * Constructs a BlockChain connected to the given list of listeners (eg,
     * wallets) and a store.
     */
    ...
    /**
     * Add a wallet to the BlockChain. Note that the wallet will be unaffected by
     * any blocks received while it
     * was not part of this BlockChain. This method is useful if the wallet has just
     * been created, and its keys
     * have never been in use, or if the wallet has been loaded along with the
     * BlockChain. Note that adding multiple
    ...
    public final void addWallet(Wallet wallet) {
        ...
        if (walletHeight != chainHeight && walletHeight > 0) {
            ...
            // This special case happens when the VM crashes because of a transaction
            // received. It causes the updated
            // block store to persist, but not the wallet. In order to fix the issue, we
            // roll back the block store to
            // the wallet height to make it look like as if the block has never been
            // received.
    ...
    /**
     * Adds a generic {@link TransactionReceivedInBlockListener} listener to the
     * chain.
     */
    ...
    /**
     * Returns the {@link BlockStore} the chain was constructed with. You can use
     * this to iterate over the chain.
     */
     ...
     * This version is used when the transactions have already been verified to
     * properly spend txOutputChanges.
     * 
     * @param storedPrev      The {@link StoredBlock} which immediately precedes
     *                        block.
     * @param header          The {@link StoredBlock} to add/update.
     * @param txOutputChanges The total sum of all changes made by this block to the
     *                        set of open transaction outputs
     *                        (from a call to connectTransactions), if in fully
     *                        verifying mode (null otherwise).
     * @return the newly created {@link StoredBlock}
     */
    ...
    /**
     * Rollback the block store to a given height. This is currently only supported
     * by {@link BlockChain} instances.
     * 
     ...
     * Should write the new head to block store and then commit any database
     * transactions
    ...
    /**
     * Called if we (possibly) previously called
     * disconnectTransaction/connectTransactions,
     ...
     * something wrong with the block an
     * exception is thrown. If the block is OK but cannot be connected to the chain
     * at this time, returns false.
     * If the block can be connected to the chain, returns true.
     * Accessing block's transactions in another thread while this method runs may
    ...
    public boolean add(FilteredBlock block) throws VerificationException, PrunedException {
        try {
            // The block has a list of hashes of transactions that matched the Bloom filter,
            // and a list of associated
            // Transaction objects. There may be fewer Transaction objects than hashes, this
            // is expected. It can happen
            // in the case where we were already around to witness the initial broadcast, so
            // we downloaded the
            // transaction and sent it to the wallet before this point (the wallet may have
            // thrown it away if it was
            // a false positive, as expected in any Bloom filtering scheme). The filteredTxn
            // list here will usually
            // only be full of data when we are catching up to the head of the chain and
            // thus haven't witnessed any
            // of the transactions.
    ...
    /**
     * Whether or not we are maintaining a set of unspent outputs and are verifying
     * all transactions.
     * Also indicates that all calls to add() should provide a block containing
     * transactions
     */
    protected abstract boolean shouldVerifyTransactions();

    /**
     * Connect each transaction in block.transactions, verifying them as we go and
     * removing spent outputs
     * If an error is encountered in a transaction, no changes should be made to the
     * underlying BlockStore.
     * and a VerificationException should be thrown.
     * Only called if(shouldVerifyTransactions())
     * 
     * @throws VerificationException if an attempt was made to spend an
     *                               already-spent output, or if a transaction
     *                               incorrectly solved an output script.
     ...
     *         outputs.
     */
    protected abstract TransactionOutputChanges connectTransactions(int height, Block block, StoredBlock storedPrev)
            throws VerificationException, BlockStoreException;

    /**
     * Load newBlock from BlockStore and connect its transactions, returning changes
     * to the set of unspent transactions.
     * If an error is encountered in a transaction, no changes should be made to the
     * underlying BlockStore.
     * Only called if(shouldVerifyTransactions())
     * 
     * @throws PrunedException       if newBlock does not exist as a
     *                               {@link StoredUndoableBlock} in the block store.
     * @throws VerificationException if an attempt was made to spend an
     *                               already-spent output, or if a transaction
     *                               incorrectly solved an output script.
     * @throws BlockStoreException   if the block store had an underlying error or
     *                               newBlock does not exist in the block store at
     *                               all.
     * @return The full set of all changes made to the set of open transaction
     *         outputs.
     */
    protected abstract TransactionOutputChanges connectTransactions(StoredBlock newBlock, StoredBlock storedPrev)
    ...
    private boolean add(Block block, boolean tryConnecting,
        ...
        // TODO: Use read/write locks to ensure that during chain download properties
        // are still low latency.
        lock.lock();
        try {
            // Quick check for duplicates to avoid an expensive check further down (in
            // findSplit). This can happen a lot
            // when connecting orphan transactions due to the dumb brute force algorithm we
            // use.
            ...
            // If we want to verify transactions (ie we are running with full blocks),
            // verify that block has transactions
            ...
            int height = Block.BLOCK_HEIGHT_UNKNOWN;
            final EnumSet<Block.VerifyFlag> flags;

            // Prove the block is internally valid: hash is lower than target, etc. This
            // only checks the block contents
            // if there is a tx sending or receiving coins using an address in one of our
            // wallets. And those transactions
            // are only lightly verified: presence in a valid connecting block is taken as
            // proof of validity. See the
            ...
            if (storedPrev == null) {
                if (ALLOW_MISSING_PARENTS) {
                    if (FAST_API_10POW_ENABLED) {
                        log.info("FAST_API_10POW: Block {} has no known parent. Assuming connection to chain head.",
                                block.getHashAsString());
                    } else {
                        log.warn("Block {} has no known parent. Assuming connection to chain head.",
                                block.getHashAsString());
                    }
                    ...
                    if (e.getMessage().contains("Network provided difficulty bits do not match")) {
                        log.warn(
                                "FAST_API_10POW: difficulty mismatch at height={}, expected/actual details in exception. "
                                        +
                                        "Accepting header and continuing (security relaxed in FAST_API_10POW). Error: {}",
                                height, e.getMessage());
                    } else {
                        throw e;
                    }
    ...
    private boolean shouldSkipHeaderVerification(int height) {
        return SKIP_HEADER_VERIFICATION_AFTER_HEIGHT >= 0 &&
                height >= SKIP_HEADER_VERIFICATION_AFTER_HEIGHT;
    }

    /**
     * Returns the hashes of the currently stored orphan blocks and then deletes
     * them from this objects storage.
     * Used by Peer when a filter exhaustion event has occurred and thus any orphan
    ...
    // expensiveChecks enables checks that require looking at blocks further back in
    // the chain
    ...
    private void connectBlock(final Block block, StoredBlock storedPrev, boolean expensiveChecks,
        ...
        if (storedPrev.equals(head)) {
            ...
            // BIP 66 & 65: Enforce block version 3/4 once they are a supermajority of
            // blocks
            ...
            // This block connects to the best known block, it is a normal continuation of
            // the system.
            ...
            // This block connects to somewhere other than the top of the best known chain.
            // We treat these differently.
            //
            // Note that we send the transactions to the wallet FIRST, even if we're about
            // to re-organize this block
            // to become the new best chain head. This simplifies handling of the re-org in
            // the Wallet class.
            ...
            if (haveNewBestChain) {
                ...
                if (splitPoint.equals(newBlock)) {
                    ...
                    // when we receive a block
                    // that we already saw and linked into the chain previously, which isn't the
                    // chain head.
            ...
            // We may not have any transactions if we received only a header, which can
            // happen during fast catchup.
            // If we do, send them to the wallet but state that they are on a side chain so
            // it knows not to try and
    ...
    private void informListenersForNewBlock(final Block block, final NewBlockType newBlockType,
        ...
        // Notify the listeners of the new block, so the depth and workDone of stored
        // transactions can be updated
        // (in the case of the listener being a wallet). Wallets need to know how deep
        // each transaction is so
    ...
    private static void informListenerForNewTransactions(Block block, NewBlockType newBlockType,
        ...
        if (block.transactions != null) {
            // If this is not the first wallet, ask for the transactions to be duplicated
            // before being given
            // to the wallet when relevant. This ensures that if we have two connected
            // wallets and a tx that
            // is relevant to both of them, they don't end up accidentally sharing the same
            // object (which can
            // result in temporary in-memory corruption during re-orgs). See bug 257. We
            // only duplicate in
            // the case of multiple wallets to avoid an unnecessary efficiency hit in the
            // common case.
     ...
     * Only called if(shouldVerifyTransactions())
     * 
     * @throws PrunedException     if block does not exist as a
     *                             {@link StoredUndoableBlock} in the block store.
     * @throws BlockStoreException if the block store had an underlying error or
     ...
     * 
     * if (shouldVerifyTransactions)
     * Either newChainHead needs to be in the block store as a FullStoredBlock, or
     * (block != null && block.transactions != null)
     */
    private void handleNewBestChain(StoredBlock storedPrev, StoredBlock newChainHead, Block block,
        ...
        // required.
        //
        // Firstly, calculate the block at which the chain diverged. We only need to
        // examine the
        ...
        // Then build a list of all blocks in the old part of the chain and the new
        // part.
        ...
        // Disconnect each transaction in the previous best chain that is no longer in
        // the new best chain
        ...
        if (shouldVerifyTransactions()) {
            for (StoredBlock oldBlock : oldBlocks) {
                try {
                    ...
                    // We threw away the data we need to re-org this deep! We need to go back to a
                    // peer with full
                    // block contents and ask them for the relevant data then rebuild the indexs. Or
                    // we could just
                    // give up and ask the human operator to help get us unstuck (eg, rescan from
                    // the genesis block).
            ...
            for (Iterator<StoredBlock> it = newBlocks.descendingIterator(); it.hasNext();) {
                ...
                if (expensiveChecks && cursorBlock
                        .getTimeSeconds() <= getMedianTimestampOfRecentBlocks(cursor.getPrev(blockStore), blockStore))
        ...
        // Now inform the listeners. This is necessary so the set of currently active
        // transactions (that we can spend)
        // can be updated to take into account the re-organize. We might also have
        // received new coins we didn't have
    ...
    /**
     * Locates the point in the chain at which newStoredBlock and chainHead diverge.
     * Returns null if no split point was
     * found (ie they are not part of the same chain). Returns newChainHead or
     * chainHead if they don't actually diverge
    ...
    private static StoredBlock findSplit(StoredBlock newChainHead, StoredBlock oldChainHead,
        ...
        while (!currentChainCursor.equals(newChainCursor)) {
            if (currentChainCursor.getHeight() > newChainCursor.getHeight()) {
                StoredBlock prev = currentChainCursor.getPrev(store);
                if (prev == null) {
                    log.warn("Orphan chain while finding split: block={}, height={}, prev={}",
                            currentChainCursor.getHeader().getHash(), currentChainCursor.getHeight(),
                            currentChainCursor.getHeader().getPrevBlockHash());
                    return null;
                }
                currentChainCursor = prev;
            } else {
                StoredBlock prev = newChainCursor.getPrev(store);
                if (prev == null) {
                    log.warn("Orphan chain while finding split: block={}, height={}, prev={}",
                            newChainCursor.getHeader().getHash(), newChainCursor.getHeight(),
                            newChainCursor.getHeader().getPrevBlockHash());
                    return null;
                }
                newChainCursor = prev;
            }
    ...
    private static void sendTransactionsToListener(StoredBlock block, NewBlockType blockType,
        ...
        for (Transaction tx : transactions) {
            try {
                ...
                // We don't want scripts we don't understand to break the block chain so just
                // note that this tx was
    ...
    /**
     * For each block in orphanBlocks, see if we can now fit it on top of the chain
     * and if so, do so.
     */
    private void tryConnectingOrphans() throws VerificationException, BlockStoreException, PrunedException {
        ...
        // of the round try again until
        // we can't fit anything else on the top.
        //
        // This algorithm is kind of crappy, we should do a topo-sort then just connect
        // them in order, but for small
    ...
    /**
     * Returns the block at the head of the current best chain. This is the block
    ...
    /**
     * An orphan block is one that does not connect to the chain anywhere (ie we
     * can't find its parent, therefore
     * it's an orphan). Typically this occurs when we are downloading the chain and
     * didn't reach the head yet, and/or
     * if a block is solved whilst we are downloading. It's possible that we see a
     * small amount of orphan blocks which
     * chain together, this method tries walking backwards through the known orphan
     * blocks to find the bottom-most.
     *
     * @return from or one of froms parents, or null if "from" does not identify an
     *         orphan block
     */
    @Nullable
    ...
    /**
     * Returns an estimate of when the given block will be reached, assuming a
     * perfect 10 minute average for each
     * block. This is useful for turning transaction lock times into human readable
     * times. Note that a height in
     * the past will still be estimated, even though the time of solving is actually
     * known (we won't scan backwards
    ...
    /**
     * Returns a future that completes when the block chain has reached the given
     * height. Yields the
     * {@link StoredBlock} of the block that reaches that height first. The future
     * completes on a peer thread.
     */
     ...
     *
     * count includes filtered transactions, transactions that were passed in and
     * were relevant
     * and transactions that were false positives (i.e. includes all transactions in
     * the block).
     */
    protected void trackFilteredTransactions(int count) {
        ...
        //
        // This is slightly off because we are applying false positive tracking before
        // non-FP tracking,
        // which counts FP as if they came at the beginning of the block. Assuming
        // uniform FP
        // spread in a block, this will somewhat underestimate the FP rate (5% for 1000
        // tx block).
    ...
    void trackFalsePositives(int count) {
        // Track false positives in batch by adding alpha to the false positive estimate
        // once per count.
